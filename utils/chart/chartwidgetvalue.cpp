#include "chartwidgetvalue.h"
#include <QDebug>
#include <QApplication>

ChartWidgetValue::ChartWidgetValue(QWidget *parent) : QtCharts::QChartView(parent)
{
...
void ChartWidgetValue::slt_drawLine(QMap<QString,double> mapValue)
{
...
void ChartWidgetValue::createChart()
{
    if(nullptr == axisX)
    {
    ...
    axisX->setLabelFormat("%d");                        //设置时间显示格式
...
//    axisY->setGridLinePen(QColor(255,255,255));
    ...
    axisX->setMin(0);       //系统当前时间的前一秒
    axisX->setMax(1000);            //系统当前时间
...
void ChartWidgetValue::createSeries(QString objName, QColor color)
{
    if(m_seriesMap.find(objName) == m_seriesMap.end())
    {
...
void ChartWidgetValue::setYTrickCount(int count)
{
    if(nullptr != axisY)
    {
