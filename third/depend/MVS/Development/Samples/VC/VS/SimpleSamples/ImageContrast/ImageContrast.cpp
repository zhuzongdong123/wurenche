...
int main()
{
    ...
    do 
    {
        ...
        if (nRet == MV_OK)
        {
			...
			pDstData = (unsigned char *)malloc(sizeof(unsigned char) * (stImageInfo.stFrameInfo.nFrameLenEx));
            if (pDstData == NULL)
            {
			...
			stContrastParam.nWidth = stImageInfo.stFrameInfo.nExtendWidth;
			stContrastParam.nHeight = stImageInfo.stFrameInfo.nExtendHeight;
			stContrastParam.enPixelType = stImageInfo.stFrameInfo.enPixelType;
			stContrastParam.pSrcBuf = stImageInfo.pBufAddr;
			stContrastParam.nSrcBufLen = stImageInfo.stFrameInfo.nFrameLenEx;

			stContrastParam.pDstBuf = pDstData;
			stContrastParam.nDstBufSize = stImageInfo.stFrameInfo.nFrameLenEx;
			...
			fwrite(stImageInfo.pBufAddr, 1, stImageInfo.stFrameInfo.nFrameLenEx, fp);
            fclose(fp);
			fp = NULL;
			...
			fwrite(pDstData, 1, stImageInfo.stFrameInfo.nFrameLenEx, fp);
            fclose(fp);
